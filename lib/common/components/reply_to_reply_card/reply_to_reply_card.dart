...
class ReplyToReplyCard extends StatelessWidget {
  const ReplyToReplyCard({
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<ReplyToReplyCardModel>(
      create: (context) => ReplyToReplyCardModel(),
      child: Consumer<ReplyToReplyCardModel>(
                              ...
                              Navigator.push<void>(
                                ...
                                MaterialPageRoute(
                                  builder: (context) => CommonPostsPage(
                                    title: replyToReply.emotion,
                                    type: CommonPostsType.searchResultPosts,
                                    searchWord: replyToReply.emotion,
                                  ),
                                ),
                              );
                            },
                            child: replyToReply.emotion != null &&
                                    kEmotionIcons[replyToReply.emotion] != null
                                ? Image.asset(
                                    kEmotionIcons[replyToReply.emotion]!,
                                  )
                                : const FlutterLogo(),
                          ),
                        ),
                        ...
                        children: [
                          IconButton(
                            icon: Icon(
                              ...
                              color: replyToReply.isEmpathized
                                  ? Colors.pinkAccent
                                  : kGrey,
                            ),
                            onPressed: () async {
                              if (replyToReply.isEmpathized) {
                                // 下二行はワカル１回のみできるとき用
                                model.turnOffEmpathyButton(replyToReply);
                                await model.deleteEmpathizedPost(replyToReply);
                              } else {
